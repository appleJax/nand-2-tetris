...
describe("handleSymbol", () => {
  ...
  it("should record a new symbol entry if one does not currently exist", () => {
    const symbol = "someSymbol";
    const SYMBOLS = {};
    const SYMBOLIC_ADDRESS = {
      value: 16,
    };
    const result = handleSymbol(symbol, SYMBOLS, SYMBOLIC_ADDRESS);

    expect(result).toEqual(SYMBOLS[symbol]);
    expect(SYMBOLS[symbol]).toEqual(16);
  });

  it("should accept a nextAddress parameter for storing a new symbol", () => {
    const symbol1 = "someSymbol";
    const symbol2 = "anotherSymbol";
    const SYMBOLS = {};
    const SYMBOLIC_ADDRESS = {
      value: 37,
    };
    const someSymbol = handleSymbol(symbol1, SYMBOLS, SYMBOLIC_ADDRESS);
    const anotherSymbol = handleSymbol(symbol2, SYMBOLS, SYMBOLIC_ADDRESS);

    expect(someSymbol).toEqual(SYMBOLS[symbol1]);
    expect(SYMBOLS[symbol1]).toEqual(37);

    expect(anotherSymbol).toEqual(SYMBOLS[symbol2]);
    expect(SYMBOLS[symbol2]).toEqual(38);
  });
});
...
describe("cleanProgramAndRecordJumpSymbols", () => {
  it("should remove whitespace and comments", () => {
    ...
    expect(cleanedProgram).toEqual(["line1", "line2"]);
  });
  ...
  it("should record JUMP symbols with the correct line numbers", () => {
    ...
    const cleanedProgram = cleanProgramAndRecordJumpSymbols(
      rawProgram,
      SYMBOLS
    );
    expect(cleanedProgram).toEqual(["line0", "line1", "line2"]);
...
describe("translateA", () => {
  ...
  it("should record new symbols at the next free address", () => {
    const SYMBOLS = {};
    const symbolicAddress = {
      value: 5,
    };

    expect(translateA("newSymbol", SYMBOLS, symbolicAddress)).toEqual(
      "0000000000000101"
    );
  });
});
...
describe("translateC", () => {
  ...
  it("should accept a custom command mapping", () => {
    ...
    expect(translateC("dest=comp;jump", customCommands)).toEqual(
      "111CCCDDDJJJ"
    );
  });
});
